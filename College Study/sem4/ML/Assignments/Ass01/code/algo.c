...
void classifyAndValidateSet(point *training_set, int training_size, point *testing_set, int testing_size, int k, int p, int t_start, int t_end)
{
    int correct = 0, wrong = 0;

    for (int i = 0; i < testing_size; ++i)
    {
        bool check = classifyAndValidate(training_set, testing_set[i], training_size, k, p, t_start, t_end);
        if (check)
            ++correct;
        else
            ++wrong;
...
void performRFoldCrossValidation(point *point_set, int size, int k, int p)
{
    ...
    makeAndForwardSet(point_set, size, folds[0], folds[1], k, p);
}
...
int main()
{
    NO_OF_FEATURES = 7;
    NO_OF_CLASSES = 3;
    int lines = get_lines("rand_seeds_dataset.txt");

    point *point_set = malloc(lines * sizeof(point));

    point *pset = read_from_file("rand_seeds_dataset.txt", point_set);
    ...
    performRFoldCrossValidation(point_set, lines, 3, 1);
    // for (int i = 0; i < K; ++i)
    // {
