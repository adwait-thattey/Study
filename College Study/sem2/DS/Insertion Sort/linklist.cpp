...
class linklist {
	...
	public :
		...
		bool swap(node *ptr1 , node*ptr2);
		bool move(node *ptr , node *pos);


};

bool linklist::move(node *ptr , node *pos=NULL) {
	// moves the node to position after pos
	...
	ptr->next->previous = ptr->previous;
	ptr->next = pos->next;
	if(pos->next) pos->next->previous = ptr;
	ptr->previous = pos;
	pos->next = ptr;
	return true;
...
bool linklist::swap(node *ptr1 , node*ptr2) {
	...
	if(ptr1->next==ptr2) {
		if(ptr1->previous) ptr1->previous->next = ptr2;
		if(ptr2->next) ptr2->next->previous = ptr1;
	...
	if(ptr2->next==ptr1) {
		return swap(ptr2,ptr1);
	}


	node *temp;
	
	if(ptr1->next) ptr1->next->previous = ptr2;
	if(ptr1->previous) ptr1->previous->next = ptr2;
	if(ptr2->next) ptr2->next->previous = ptr1;
	if(ptr2->previous) ptr2->previous->next = ptr1;
	temp = ptr1->next;
	ptr1->next = ptr2->next;
	ptr2->next = temp;
	temp = ptr1->previous;
